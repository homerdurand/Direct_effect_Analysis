...
def fit_svd(X, Y, Z, center=False, scale=False, tol=None, rank=None, intercept=False):
    if X.ndim == 1:
        X = X.reshape(-1, 1)
...
def fit_chow(X, Y, Z, reg_full, reg_res, solver='Roy', alpha=1e-5):
    n = X.shape[0]
...
def fit_optimal_detector(X, Y, Z, reg_full, reg_res, alpha=1e-5):
    n = X.shape[0]
    reg_full.fit(np.hstack((X, Z)), Y)
    reg_res.fit(Z, Y)
    RSS_full = Y - reg_full.predict(np.hstack((X, Z)))
...
class DirectEffectAnalysis:
    def __init__(self, type ='Roy', regressor_0=LinearRegression(), regressor_1=LinearRegression(), alpha=1e-5):
    ...
    def fit(self, X, Y, Z):
        if self.type == 'Roy' or self.type == 'EVD':
            self.u, self.W = fit_chow(X, Y, Z, self.regressor_0, self.regressor_1, self.type, self.alpha)
        elif self.type == 'PLS' :
            self.mva = fit_GCM(X, Y, Z, self.regressor_0, self.regressor_1, PLSCanonical())
        elif self.type == 'genCCA' :
            ...
            self.pca.fit(Y)
        elif self.type == 'OptiDet':
            self.u, self.W, self.reg_full = fit_optimal_detector(X, Y, Z, self.regressor_0, self.regressor_1, self.alpha)


    def transform(self, X, Y, Z):
        if self.type == 'Roy' or self.type == 'EVD' or self.type == 'RoyS' or self.type == 'OptiDet':
            return Y @ self.W[:, 0]
        elif self.type == 'genCCA':
